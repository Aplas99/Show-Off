import { CreateItemPayload, validateCreateItem } from "@/lib/createItemValidation";
...
const BarcodeLookupAPIKey = process.env.EXPO_PUBLIC_BARCODE_LOOKUP_API_KEY || "";
...
async function fetchFromBarcodeLookup(
    ...
    if (!BarcodeLookupAPIKey) {
        console.warn("[API] ⚠️ Barcode Lookup API key is missing. Skipping request.");
        return [];
    }
    ...
    if (filters?.manufacturer?.trim()) urlParams.append("manufacturer", filters.manufacturer.trim());
    if (filters?.category?.trim()) urlParams.append("category", filters.category.trim());
    ...
    return rawItems.map((item: any) => ({
        ...
        price: item.stores?.[0]?.price ? parseFloat(item.stores[0].price) : undefined,
...
async function fetchFromUPCItemDB(query: string): Promise<Product[]> {
    ...
    const response = await fetch(`https://api.upcitemdb.com/prod/trial/${endpoint}`, {
        method: "POST",
...
export function useProductSearch(params: SearchParams) {
    return useInfiniteQuery({
        ...
        queryFn: async ({ pageParam = 1 }) => {
            ...
            const items = await fetchFromBarcodeLookup(params.query, pageParam as number, params);
            return { items };
        },
        initialPageParam: 1,
